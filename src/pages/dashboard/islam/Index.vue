<template>
    <PageToolbar title="Five Pillars of Islam" v-model:modelValue="search" :showSearch="true" :showView="false"
        :showExcel="true" :showAdd="true" :useModal="true" @add="openAddModal" />
      <div>
        <FivePillarIslamRowCard :pillars="filteredPillars" @edit="editPillar" @delete="deletePillar" />
      </div>
      <Dialog v-model:visible="showModal" modal :header="modalHeader" :style="{ width: '50rem' }"
        :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <FivePillarIslamFrom :isEditMode="isEditMode" :fivePillarIslamData="selectedFivePillarIslam"
...
import FivePillarIslamRowCard from '@/components/dashboard/islam/FivePillarIslamRowCard.vue'
import FivePillarIslamFrom from '@/components/dashboard/islam/FivePillarIslamFrom.vue'
...
const selectedFivePillarIslam = ref(null)

// Sample data for Five Pillars of Islam
const pillars = ref([
    {
        id: 1,
        name: "Shahada",
...
const modalHeader = computed(() => {
    return isEditMode.value ? 'Edit Five Pillar of Islam' : 'Add New Five Pillar of Islam'
})
...
const openAddModal = () => {
    ...
    selectedFivePillarIslam.value = null
    showModal.value = true
}

const editPillar = (pillar) => {
    isEditMode.value = true
    selectedFivePillarIslam.value = { ...pillar }
    showModal.value = true
}
