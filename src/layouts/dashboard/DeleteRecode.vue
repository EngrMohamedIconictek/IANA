<template>
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '28rem' }"
        ...
        <div class="text-center p-2">
            ...
            <p class="text-gray-600 mb-1">Are you sure you want to delete this record?</p>
            <p class="text-sm text-gray-500">This action cannot be undone.</p>
        </div>

        <template #footer>
            <div class="flex justify-center items-center gap-3 p-4 ">
                <Button type="button" label="Cancel" severity="secondary" @click="visible = false" outlined size="large"
                    class="min-w-[180px] h-12 rounded-xl font-semibold text-base hover:-translate-y-0.5 hover:shadow-lg transition-all duration-200" />
                <Button type="button" label="Delete" severity="danger" @click="confirmDelete" :loading="loading"
                    size="large"
                    class="min-w-[180px] h-12 rounded-xl font-semibold text-base bg-gradient-to-r from-red-500 to-red-600 border-none shadow-lg shadow-red-500/30 hover:from-red-600 hover:to-red-700 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-red-500/40 transition-all duration-200" />
            </div>
        </template>
    </Dialog>
</template>
